...
class mega_co_nz extends DownloadClass {
	...
	private function doApiReq($atrr, $node='') {
		...
		$page = $this->GetPage('https://lu1.api.mega.nz/cs?id=' . ($this->seqno++) . (!empty($node) ? "&n=$node" : '') . (!empty($this->cookie['sid']) ? "&sid={$this->cookie['sid']}" : ''), 0, json_encode((!empty($atrr[0]) && is_array($atrr[0])) ? $atrr : array($atrr)), "https://mega.nz/\r\nContent-Type: application/json");
